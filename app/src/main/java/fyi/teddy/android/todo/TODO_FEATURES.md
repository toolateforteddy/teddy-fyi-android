...
- [x] **Rich Recurrence Rules (RFC 5545)**: Support complex recurrence patterns (e.g., "every Tuesday and Thursday", "every month") using standardized iCalendar RRULE strings. Replaced legacy integer column with a single `recurrenceRule: String?` column in `todo_items`, complete with UI selection options ("Every Day", "Every Week", "Tuesday & Thursday", "Monday, Wednesday & Friday", "Every Month", or "Custom Days") and human-readable recurrence formatting. Fully verified with `migrate21To22` database migration tests.
- [x] **State-Driven Overlays (Unified Dialog Coordinator)**: Refactored individual dialog booleans inside the task row component into a single, unified `activeOverlay` state powered by a sealed interface (`ActiveRowOverlay`). Eliminates duplicate modal rendering conflicts and standardizes state transitions.
- [x] **Unidirectional MVI Flow (Intent Dispatcher)**: Unified 7 redundant task action lambdas into a single, context-free intent dispatcher (`onIntent: (TodoItemIntent) -> Unit`) driven by a clean, sealed interface (`TodoItemIntent`). Greatly simplifies Compose previews and encapsulates user-interaction logging.
- [x] **Interactive Drag-to-Reorder Gestures**: Implemented a fluid, context-aware gesture engine on the task rows using `detectDragGesturesAfterLongPress`. Long-pressing and dragging vertically when in Edit Mode instantly shifts list positions, whereas long-pressing in Standard Mode opens the overflow edit menu.
- [x] **Decoupled Position Use Cases (Sequencing Engine)**: Extracted all sibling lookup index bounds calculations and position swapping math out of ViewModels and into pure, isolated interactor classes (`MoveItemUpUseCase`, `MoveItemDownUseCase`, `MoveItemToTopUseCase`, `MoveItemToBottomUseCase`). Ensures 100% testable and highly reusable business logic.
...
- [x] **Rich Recurrence Rules (RFC 5545)**: Support complex recurrence patterns (e.g., "every Tuesday and Thursday") using iCalendar RRULE strings. Requires replacing current recurrence columns with a single `recurrenceRule: String?` column.
